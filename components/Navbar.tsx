  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[#050505]/95 backdrop-blur-md border-b border-gray-800'
          : 'bg-transparent'
      }`}
    >
      ...
      {/* Mobile menu */}
      <div
        className={`md:hidden transition-all duration-300 overflow-hidden bg-[#0d0d0d] border-b border-gray-800 ${
          ...
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                ...
                className="text-gray-300 hover:text-white transition-colors duration-200 text-sm font-medium block"
              >
                {link.label}
              </a>
            </li>
